export class TableTreeConfigModel implements ITableTreeConfig {
  ...
  convertDataRawToDataTree(dataRaw: any[]): any[] {
    ...
    const result = dataRaw.filter(x => !x[this.keyParentId]);
  ...
  convertTreeToList(root: any): any[] {
    ...
    const array: any[] = [];
    const hashMap = {};
    stack.push({ ...root, level: 0, expand: this.collapseDefault ? root.children.length > 0 : false });
    ...
    while (stack.length !== 0) {
      ...
      if (node.children) {
        for (let i = node.children.length - 1; i >= 0; i--) {
          // tslint:disable-next-line: no-non-null-assertion
          stack.push({ ...node.children[i], level: node.level! + 1, expand: this.collapseDefault ? node.children[i].children.length > 0 : false, parent: node });
        }
      }
    }
