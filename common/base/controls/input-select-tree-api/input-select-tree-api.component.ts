...
@Component({
  selector: 'input-select-tree-api',
  templateUrl: './input-select-tree-api.component.html',
  styleUrls: ['./input-select-tree-api.component.scss'],
  encapsulation: ViewEncapsulation.None,
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => InputSelectTreeApiComponent),
      multi: true,
    },
  ],
})
export class InputSelectTreeApiComponent
  implements OnInit, OnDestroy, ControlValueAccessor, OnChanges
{
  ...
  public controlText: string | null | undefined = null;
  ...
  async getData(page: number = 1, textSearch: string = ''): Promise<void> {
    ...
    if (!rs.errors) {
      ...
      if (textSearch) {
        // nếu tìm kiếm thì cứ mở hết level
        getMinLevel = 1;
      ...
      if (this.textSearch) {
        ...
        const itemFirst = dataRaw.filter((x) => !x.disabled)[0];
        this.setSelectFirst = true;
        this.selectListSelection.clear();
        this.selectListSelection.toggle(itemFirst.key);
        this.controlText = await this.showTextValue(
          itemFirst.name,
          itemFirst.key
        );
        this.controlValue = itemFirst.key;
        this.eventBaseChange(this.controlValue);
        this.eventOnChange.emit({
          label: this.controlText,
    ...
    if (this.controlValue) {
      ...
      if (!rs.errors && rs.data) {
        ...
        if (this.listItemBonus.length > 0) {
          ...
          this.selectListSelection.toggle(this.controlValue);
        }
      }
    } else {
  ...
  onClear(): void {
    ...
    this.controlText = '';
    this.eventBaseChange(this.controlValue);
    this.eventOnChange.emit({ label: null, value: null });
  }
  ...
  async selectItem(item: any) {
    ...
    this.selectListSelection.clear();
    this.selectListSelection.toggle(item.key);

    this.controlText = await this.showTextValue(item.name, item.key);
    this.controlValue = item.key;
