...
export class InputMonthComponent implements OnInit, AfterViewInit, ControlValueAccessor, Validator, OnChanges {
  constructor(
  ...
  validate(c: FormControl) {
    if (c.valid && this.controlValue) {
      ...
      if (this.max) {
        if (this.max && +(this.max.toNumberYYYYMMDD()?.toString().substring(0, vl)!) < this.getValueNumber()!) {
          return { error: `Ngày không được lớn hơn ${this.datePipe.transform(this.max, 'dd/MM/yyyy')}` }
        }
      }
      if (this.min) {
        if (this.min && +(this.min.toNumberYYYYMMDD()?.toString().substring(0, vl)!) > this.getValueNumber()!) {
          return { error: `Ngày không được nhỏ hơn ${this.datePipe.transform(this.min, 'dd/MM/yyyy')}` }
        }
      }
    }
    return null;
