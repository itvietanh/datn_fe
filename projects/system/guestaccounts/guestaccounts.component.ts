...
import { GuestAccountsService } from 'common/share/src/service/application/hotel/guestaccounts.service';

@Component({
  selector: 'app-guestaccounts',
  templateUrl: './guestaccounts.component.html',
  styleUrls: ['./guestaccounts.component.scss'],
})
export class GuestAccountsComponent implements OnInit {
  ...
  constructor(
    ...
    private GuestAccountsService: GuestAccountsService,
  ...
  async getData(paging: PagingModel = { page: 1, size: 20 }) {
    ...
    const rs = await this.GuestAccountsService.getPaging(params).firstValueFrom();
  ...
  async handlerOpenDialog(mode: string = DialogMode.add, item: any = null) {
    const dialog = this.dialogService.openDialog(
      async (option) => {
        ...
        option.component = GuestAccountsDetailComponent; // open component;
        option.inputs = {
          uuid: item?.uuid,
          mode: mode,
        };
